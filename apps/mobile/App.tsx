...
function AppTabs() {
  const [fontsLoaded] = useFonts({
    'ITC Avant Garde Gothic Bold': require('./assets/media-library/fonts/ITC Avant Garde Gothic/ITC Avant Garde Gothic Bold/ITC Avant Garde Gothic Bold.otf'),
    'ITC Avant Garde Gothic': require('./assets/media-library/fonts/ITC Avant Garde Gothic/ITC Avant Garde Gothic Medium/ITC Avant Garde Gothic Medium.otf'),
    'DM Sans': require('./assets/media-library/fonts/DM_Sans/DMSans-VariableFont.ttf'),
  });
  if (!fontsLoaded) {
  ...
  return (
    <SocialTab.Navigator
        ...
        screenOptions={({ route }) => ({
          headerTitleStyle: { fontSize: 28, fontWeight: '400', paddingBottom: 20, backgroundColor: 'transparent', fontFamily: 'ITC Avant Garde Gothic' },
    ...
    <NavigationContainer>
      <Stack.Navigator>
        <Stack.Screen name="Tabs" component={AppTabs} options={({ route }) => ({
          ...
          headerTitleStyle: { fontFamily: 'ITC Avant Garde Gothic' }
        })}/>
        ...
        <Stack.Screen
          ...
          options={() => ({
            headerTransparent: true,
            headerTitleStyle: { fontSize: 28, fontWeight: '400', fontFamily: 'ITC Avant Garde Gothic' },
            headerBackground: () => (
              <View style={{ flex: 1, backgroundColor: 'rgba(255,255,255,0.6)' }} />
            ),
          })}
        />
        <Stack.Screen name="Results" component={ResultsScreen} options={({route}) => ({title: route.params.title ?? 'Results', headerTransparent: true, headerTitleStyle: { fontSize: 28, fontWeight: '400', fontFamily: 'ITC Avant Garde Gothic'}})}/>
      </Stack.Navigator>
    </NavigationContainer>
  );
}// export default function App() {
